...
const Trade = (props) => {
  ...
  const fetchTrade = async () => {
    try {
      const response = await fetch(`http://localhost:5000/trade/${_id}`);
  ...
  const acceptTrade = async () => {
    try {
      ...
      // );
      const response = await fetch(`http://localhost:5000/accepttrade/${_id}`, {
        method: "GET",
        //headers: headers,
        //body: JSON.stringify({}),
      });
  ...
  const rejectTrade = async () => {
    try {
      ...
      //   "Basic " + base64.encode(email + ":" + password)
      // );
      const response = await fetch(`http://localhost:5000/rejecttrade/${_id}`, {
        method: "DELETE",
        headers: headers,
      });
      const json = await response.json();
